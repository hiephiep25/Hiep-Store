            ...
            <q-card class="my-card bg-white q-pa-md">
                <div class="row justify-center">
                    <div class="col">
                        ...
                        <div>
                            <strong>Loại:</strong> {{ onlineOrder.type }}
                        </div>
                        <div>
                            <strong>Hình thức thanh toán:</strong> {{ onlineOrder.payment_type }}
                        </div>
                        <div>
                            <strong>Tổng tiền:</strong> {{ onlineOrder.total }} VND
                        </div>
                        <div>
                            <strong>Thời gian mua:</strong> {{ formatDate(onlineOrder.created_at) }}
                        </div>
                        <div>
                            <strong>Khách hàng:</strong> {{ user.name }} - {{ user.email }}
                        </div>
                        <div>
                            <strong>Địa chỉ:</strong> {{ onlineOrder.online_order.customer_address }}
                        </div>
                        <div>
                            <strong>Liên hệ:</strong> {{ onlineOrder.online_order.customer_phone }}
                        </div>
                        <h5>Các sản phẩm</h5>
