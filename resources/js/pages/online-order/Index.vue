                    ...
                    <div class="col-12 q-mt-md">
                        <q-markup-table :separator="separator" flat bordered>
                            <q-table flat bordered virtual-scroll no-data-label="không có dữ liệu"
                                ...
                                :pagination-label="getPaginationLabel" :rows="onlineOrders" :columns="columns"
...
const from = ref("");
const to = ref("");
const { onlineOrders, pagination } = storeToRefs(onlineOrderStore);
...
const onRequest = async ({ pagination }) => {
    await onlineOrderStore.getOnlineOrders({
        from: from.value,
        to: to.value,
...
const resetSearch = () => {
    from.value = "";
    to.value = ""
    onlineOrderStore.getOnlineOrders();
};

const handleInfo = (onlineOrder) => {
    router.push(`/online-order/${onlineOrder.order_id}`);
};
...
const navigateToRegistrationPage = () => {
    router.push({ name: 'online-order.create'});
};
